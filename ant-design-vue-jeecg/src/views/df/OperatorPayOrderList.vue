...
<!--      </a-upload>-->
      <a-switch checkedChildren="开始接单" unCheckedChildren="停止接单" @change="onChose" :defaultChecked="bizOnlineStatus"/>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getBizOnlineStatus(){
        getAction(this.url.getStatus).then( (res) => {
          if (res.success) {
            if (this.bizOnlineStatus == null){
              this.bizOnlineStatus = false
            } else {
              this.bizOnlineStatus = true
            }
          }
        })
      }
