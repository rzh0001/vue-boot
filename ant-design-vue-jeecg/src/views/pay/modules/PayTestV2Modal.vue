    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          label="金额">
          <a-input placeholder="金额" v-decorator="['amount', validatorRules.amount]" />
        </a-form-item>
        <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="产品选择">
          <select v-decorator="['productCode', validatorRules.productCode ]">
            <option v-for="option in products" v-bind:value="option.productCode">
              {{ option.productName}}
            </option>
          </select>
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="apikey">
          <a-input placeholder="apikey" v-decorator="['apikey', validatorRules.apikey]" />
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="clientId">
          <a-input placeholder="clientId" v-decorator="['clientId', validatorRules.clientId]" />
        </a-form-item>
      </a-form>
    </a-spin>
  </a-modal>
  ...
  export default {
    ...
    data () {
      return {
        ...
        validatorRules:{
          amount:{rules: [{ required: true, message: '请输入金额!' }],initialValue: 1},
          username:{rules: [{ required: true, message: '请输入用户名!' }],initialValue: "阿贵002"},
          clientId:{rules: [{ required: true, message: '请输入clientId!' }],initialValue: "payTest"},
          apikey:{rules: [{ required: true, message: '请输入apikey!' }],initialValue: "77f8a1ef73bb4b84"},
          productCode:{rules: [{ required: true, message: '请选择产品' }],initialValue: "AlipayScan"}
        },
        url: {
          add: "/api/v2/order/create",
    ...
    methods: {
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          ...
          var sign = MD5("username="+ this.model.username+ "&timestamp="+timestamp+ "&data="+ Encrypt(data,key)+"&apiKey="+key);
          var jsonObj = {
