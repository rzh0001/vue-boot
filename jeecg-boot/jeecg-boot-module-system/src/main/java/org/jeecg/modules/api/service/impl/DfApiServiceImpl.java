...
import org.jeecg.modules.df.service.impl.PayOrderServiceImpl;
...
public class DfApiServiceImpl implements IDfApiService {
    ...
    @Override
    public void assignOrderCallback(CallbackParamDTO paramDTO) {
        PayOrder order = payOrderService.findByOrderNo(paramDTO.getOrderNo());
        if (order == null) {
            log.error("签名校验失败：订单：[{}]回调异常，查询不到对应订单信息", paramDTO.getOrderNo());
            throw new RRException("订单不存在:" + paramDTO.getOrderNo());
        }
        String userName = order.getUserName();
        String deviceCode = paramDTO.getDeviceCode();
        DeviceUserEntity deviceUser = deviceUserEntityService.findByCodeAndName(userName, deviceCode);
        if (deviceUser == null) {
            log.error("签名校验失败：查询用户关联设备异常，用户名:[{}],设备code:[{}]", userName, deviceCode);
            throw new RRException("用户关联设备异常");
        }
        DeviceInfoEntity deviceInfo = deviceInfoEntityService.findByCode(deviceCode);
        if (deviceInfo == null) {
            log.error("签名校验失败：查询设备信息异常，设备code:[{}]", deviceCode);
            throw new RRException("设备信息异常");
        }
        String apiKey = deviceInfo.getApiKey();
        StringBuilder sign = new StringBuilder();
        sign.append(CallbackParamDTO.ORDER_NO_COL).append("=").append(paramDTO.getOrderNo()).append("&")
                .append(CallbackParamDTO.DEVICE_CODE).append("=").append(paramDTO.getDeviceCode()).append("&")
                .append(CallbackParamDTO.DEVICE_KEY).append("=").append(apiKey);
        String localSign = DigestUtils.md5Hex(sign.toString());
        if (!paramDTO.getSign().equals(localSign)) {
            log.error("签名校验失败：入参签名值为：[{}],本地签名值为：[{}],签名串为:[{}]", paramDTO.getSign(), localSign, sign.toString());
            throw new RRException("签名校验失败");
        }
        order.setStatus(paramDTO.getStatus().getValue());
        payOrderService.updateById(order);
    }
}
