...
public class PayOrderServiceImpl extends ServiceImpl<PayOrderMapper, PayOrder>
  ...
  @Autowired private IUserAmountEntityService userAmountService;

  @Autowired @Lazy private IApiService apiService;
    ...
    if (userAmount.compareTo(order.getAmount().add(order.getOrderFee())) < 0) {
      ...
      throw new ApiException(
          ...
          "余额不足，请及时充值！当前余额["
              + userAmount
              + "]元，本订单需["
              + order.getAmount().add(order.getOrderFee())
              + "]元");
    }
    // 扣减商户余额
    userAmountService.changeAmount(
        user.getId(), order.getAmount().negate(), order.getOrderId(), null, "5");
    String remark = "单笔固定手续费：" + order.getFixedFee() + "，交易手续费：" + order.getTransactionFee();
    userAmountService.changeAmount(
        user.getId(), order.getOrderFee().negate(), order.getOrderId(), remark, "6");
