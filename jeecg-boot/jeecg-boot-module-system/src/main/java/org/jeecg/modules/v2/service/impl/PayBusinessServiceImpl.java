 ...
 */
@Service
public class PayBusinessServiceImpl extends ServiceImpl<PayBusinessMapper, PayBusiness> implements
    IService<PayBusiness> {
    ...
    public List<PayBusiness> getBusiness(String userName,String channelCode,String productCode){
        ...
        queryWrapper.eq("user_name",userName).eq("channel_code",channelCode).eq("product_code",productCode)
            .eq("del_flag", DeleteFlagEnum.NOT_DELETE.getValue()).eq("business_active_status", BusinessActivStatusEnum.ACTIVE.getValue()).orderByDesc("last_used_time");
